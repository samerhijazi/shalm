...
- Qute templates: `dashboard.html` (shell — head/style/header/5-tab nav/JS) `{#include}`s five partials under `templates/tabs/`, one per top-level tab. Five top-level tabs, default landing tab is **Dashboard**:
  ...
  - **Operations**: **Network** subtab (channel, chaincode name+version, latest block, Orderer host + a real Healthy/Unavailable badge from `OrdererHealthService`'s `GET /healthz` call to the orderer's Operations endpoint — see Fabric section below; Peer0 Org2/State DB are still honestly "Unknown", not fabricated, since only one Fabric identity is connected), **Consistency** subtab (the detailed API-vs-Fabric comparison, moved here from the old World State tab, plus a "Run Consistency Check" action), **Tests** subtab (same test-result cards as before)
...
- The orderer's main gRPC port (7050) only serves Fabric's own `AtomicBroadcast`/`Cluster` services — no generic health check lives there. Real orderer health is checked via Fabric's separate Operations HTTP service instead: `ORDERER_OPERATIONS_LISTENADDRESS=0.0.0.0:8443` (default is `127.0.0.1:8443`, unreachable outside the pod) exposes `GET /healthz`, called by `OrdererHealthService` (plain `java.net.http.HttpClient`, no new dependency) and surfaced on Operations → Network
